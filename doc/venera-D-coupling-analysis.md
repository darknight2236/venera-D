...
- **两个同名的 `JsEngine` 单例类**：`foundation/js_engine.dart:59`（漫画源 JS 引擎）与 `utils/image.dart:196`（自定义图片处理引擎），都包 FlutterQjs、都是 factory 单例。`network/images.dart:8` import 后者、`foundation/image_provider/reader_image.dart:5` import 前者，搜索/重构时极易混淆。
...
### 3.1 层级倒置：foundation/network 反向 import UI 层（现存 4 处，原有 6 处）

| 倒置点 | 位置 | 用途 | 状态 |
|---|---|---|---|
| `foundation/comic_source/comic_source.dart:14-15` | import `pages/category_comics_page.dart` + `pages/search_result_page.dart` | 领域模型 `PageJumpTarget.jump()` 直接 new 页面 Widget 并跳转（`models.dart:537-560`）——**导航逻辑长在数据模型里** | 待改造 |
...
| `foundation/local.dart:13` | import `pages/reader/reader.dart` | 本地漫画管理器依赖阅读器页面 | 待改造 |
| `network/cloudflare.dart:9` | import `pages/webview.dart` | Cloudflare 绕过直接开 WebView 页面 | 待改造 |
| `init.dart:14-15` | import 2 个 pages 文件（原有 3 个，settings_page 已通过 `App.appUpdateUiHandler` 回调解除） | 初始化层依赖 UI | 待改造 |

补充：`foundation/context.dart:2` import `components/components.dart`（`showToast`）——foundation 对 components 的依赖，验收 grep 会命中，需单独处理（豁免或将 `showMessage` 移出 context.dart）。
...
5. **两个同名 `JsEngine` 类**（`foundation/js_engine.dart:59` vs `utils/image.dart:196`）——不是架构问题，但随时会咬人。
...
| 优先级 | 事项 | 成本 | 收益 |
|---|---|---|---|
| **P0** | **拆掉剩余 4 处层级倒置**（原 6 处，js_engine/favorites 已完成）：把 `PageJumpTarget.jump()`（models.dart:537）移到 pages 侧扩展方法；`local.dart:13`、`cloudflare.dart:9`、`init.dart:14-15` 用回调/接口反转。详见 `layer-inversion-refactor-plan.md` | 1 天 | foundation/network 恢复可独立编译，headless 和未来的测试解锁 |
| **P1** | **重命名 `utils/image.dart` 的 `JsEngine`**（如 `ImageProcessEngine`） | 半天 | 消除随时会爆的混淆 |
| **P1** | **给单例加测试接缝**：把 `final appdata = ...`、`final App = ...` 改为可覆盖（如 `static T? mockInstance` 或构造参数注入默认值），`CacheManager` 已有 `static instance` 模式可推广。不需要 DI 框架 | 1 天 | 让"为核心逻辑写回归测试"成为可能——接手停更 fork 最需要的安全网 |
