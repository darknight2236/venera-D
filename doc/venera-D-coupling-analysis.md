...
- **Settings 没有任何中间层**。设置页直接 `appdata.settings[SettingKeys.key] = value`，例如 `lib/pages/settings/reader.dart:78`、`:209`、`:211`；5 个 settings 文件（app/appearance/explore_settings/network/reader + setting_components）全部直接读写。Settings 是一个 64 个 key 的 `Map<String, dynamic>`（`appdata.dart`），key 已收敛为 `SettingKeys` 常量（✅ 2026-07），拼写错误现为编译期错误；值仍为 dynamic。
