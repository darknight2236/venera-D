...
| **P1** | **给单例加测试接缝**（🟡 样板已完成，2026-07）：已在 `Appdata` 上跑通端到端样板——`@visibleForTesting Appdata.forTesting()` 无 I/O 构造 + 从 `doInit()` 抽出纯 `loadFromJson()` + 顶层 `final appdata` 改为 getter+`@visibleForTesting` setter（对调用点透明）；首批测试见 `test/appdata_test.dart`（3 项通过）。**剩余**：按同一模式推广到 `App`（getter+setter）与 factory managers（HistoryManager/CacheManager 等，已有 static 字段，加 `@visibleForTesting debugSetInstance()` + 跳过 I/O 的测试构造或 `:memory:` sqlite）。不需要 DI 框架 | 1 天 | 让“为核心逻辑写回归测试”成为可能——接手停更 fork 最需要的安全网 |
