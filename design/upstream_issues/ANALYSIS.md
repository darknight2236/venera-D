...
| #825 点下一话一直转圈不显示 | ✅ 已在 venera-D 修复 | 与 #707/#799 同族根因但在阅读器侧：`pages/reader/images.dart` 的 `loadComicPages` 调用原无超时，源挂起即永久转圈；新增 `pages/reader/source_timeout.dart`（`runWithSourceTimeout`，30 秒，与下载管线一致），挂起转为可重试错误态；测试见 `test/reader_source_timeout_test.dart` |
| #771 配置同步后立即崩溃/启动崩溃 | ✅ 崩溃路径已被现有代码覆盖 | issue（1.6.2）的堆栈是 `DataSync.uploadData` 的 DioException 未捕获；当前 `utils/data_sync.dart` 的 `uploadData`/`downloadData` 已将全部 WebDAV 调用包在 try/catch 内，错误转为 `Res.error` + `_lastError` 展示，不再抛出；剩余小瑕疵：上传前 `dataVersion++` 失败时会空耗一个版本号，无功能影响 |
...
## 五、批量复核记录（2026-08，剩余 22 个候选）

对 ANALYSIS 其余候选批量查上游 git 史（`git log --grep` + 关键词），**本轮无新增"已修复噪音"**——噪音集中在高热度候选，剩余多为真未做或纯功能请求。
...
**批量复核给路线图的净变化**：真正值得后续动手的只有 **#433**（真 bug、与 #121 同数据层，可加测试）；#707/#799 需先验证是否源配置类；其余维持暂缓。
